...
// Cache statique agressif (HTML: 1h, assets: 7j)
...
// Cache news en mémoire (évite DB à chaque chargement de page)
let newsCache      = null;
let newsCacheStamp = 0;
const NEWS_TTL     = 15000; // 15 secondes

async function getCachedNews() {
    const now = Date.now();
    if (newsCache && now - newsCacheStamp < NEWS_TTL) return newsCache;
    newsCache      = await NewsModel.find({}).sort({ createdAt: -1 }).lean();
    newsCacheStamp = now;
    return newsCache;
}
...
io.on("connection", (socket) => {
    ...
    socket.on("getHistory", async ({ from, to }) => {
        try {
            if (!from || !to) return;
            // Limité aux 50 derniers messages pour charger rapidement
            const messages = await Message.find({
                $or: [{ from, to }, { from: to, to: from }]
...
if (APP_URL) {
    ...
    }, 14 * 60 * 1000); // ping toutes les 14 min (Render dort apres 15 min)
}
// GET /api/users — fallback HTTP pour charger la liste sans socket
...
// Warmup endpoint — préchauffe la connexion MongoDB et le cache
app.get("/api/warmup", asyncRoute(async (req, res) => {
    await getUsers();      // Précharge le cache utilisateurs
    await getCachedNews(); // Précharge le cache news
    res.json({ ok: true, users: usersCache.length, news: newsCache ? newsCache.length : 0 });
}));
