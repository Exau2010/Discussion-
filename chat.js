...
if (document.getElementById("toUser")) {
    ...
    socket.on("users", async (users) => {
        ...
        // Pré-charger tous les avatars en un seul appel batch
    ...
    async function showMessage(m, tempId) {
        ...
        if (
            ...
            if (isReaction) {
                ...
                // Détection HTML — rendu dans iframe sandboxé (sécurisé)
                const isHtml = /^\s*<[a-zA-Z]/.test(text) || (text.includes('<') && text.includes('>') && /<\/[a-zA-Z]/.test(text));

                if (isHtml && text) {

                    // Bloquer JavaScript et injections dangereuses
                    const forbidden =
                    ...
                    if (forbidden) {
                        ...
                        bubble.innerHTML = `
                            <div class="msg-error">
                                ⚠️ JavaScript interdit. Seuls HTML et CSS sont autorisés.
                            </div>
                        `;

                    } else {

                        // Iframe sandbox : HTML + CSS uniquement
                        ...
                        const baseStyle = '<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:sans-serif;font-size:14px;padding:8px;background:#fff;color:#111;overflow-x:hidden;word-break:break-word}</style>';
                        const safeDoc = metaViewport + baseStyle + text;

                        bubble.innerHTML = `
                            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;">
                                <span style="font-size:10px;letter-spacing:1px;text-transform:uppercase;color:#00e5a0;font-weight:600;">⚡ HTML</span>
                                <span style="font-size:10px;color:#6b7280;">rendu sécurisé</span>
                            </div>
                            <iframe
                                id="${iframeId}"
                                sandbox=""
                                style="width:100%;min-height:60px;border:1px solid rgba(0,229,160,0.2);border-radius:10px;background:#fff;display:block;"
                                srcdoc=""
                            ></iframe>
                        ...
                        // Injecter le contenu après rendu DOM + auto-resize
                        requestAnimationFrame(() => {
                            ...
                            iframe.onload = () => {
                                try {
                                    ...
                                    iframe.style.height = Math.min(Math.max(h + 16, 60), 500) + "px";
                                } catch {}
                            };
                        });
                    }
