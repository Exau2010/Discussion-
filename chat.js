...
if (document.getElementById("toUser")) {
    ...
    // S'enregistrer et charger l'historique dès la connexion
    socket.on("connect", () => {
    ...
    function sendMessage() {
        ...
        // Envoyer au serveur pour sauvegarde + relais
    ...
    // ===== Confirmation serveur (remplace l'id temporaire par l'id MongoDB) =====
    socket.on("messageConfirmed", ({ tempId, msg }) => {
        // Mettre à jour l'id du div affiché pour que la suppression fonctionne
        const el = document.getElementById("msg-" + tempId);
        if (el) {
            el.id = "msg-" + msg.id;
            // Mettre à jour le bouton supprimer avec le bon id
            const delBtn = el.querySelector(".del-btn");
            if (delBtn) delBtn.setAttribute("data-id", msg.id);
        }
    });

    // ===== Réception message de l'autre =====
    socket.on("privateMessage", m => {
        // Ne pas afficher si c'est notre propre message (déjà affiché en optimiste)
    ...
    function showMessage(m, tempId) {
        ...
        if (
            ...
            let html = m.text ? `<span>${m.text}</span>` : "";
            ...
            // Heure + statut vu + bouton suppression
            html += `
...
// ===== Suppression (appelée par le bouton) =====
function deleteMsg(btn) {
    const id = btn.getAttribute("data-id");
    if (!id || id.startsWith("tmp-")) return; // Pas encore confirmé par le serveur
